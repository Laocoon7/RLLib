...
public abstract class GameWindow : Grid, IDisposable
{
    ...
    #region Wrappers From Silk.NET
    /// <summary>
    /// Wraps Silk.NET.Windowing.WindowState -> RLLib.WindowState
    /// </summary>
    /// <param name="windowState"></param>
    private void OnWindowStateChanged(Silk.NET.Windowing.WindowState windowState) => OnWindowStateChanged((WindowState)windowState);
    /// <summary>
    /// Wraps Silk.NET.Input.MouseButton -> RLLib.MouseButton
    /// </summary>
    /// <param name="button"></param>
    private void OnMouseDown(Silk.NET.Input.MouseButton button) => OnMouseDown((MouseButton)button);
    /// <summary>
    /// Wraps Silk.NET.Input.MouseButton -> RLLib.MouseButton
    /// </summary>
    /// <param name="button"></param>
    private void OnMouseUp(Silk.NET.Input.MouseButton button) => OnMouseUp((MouseButton)button);
    /// <summary>
    /// Wraps Silk.NET.Input.MouseButton -> RLLib.MouseButton
    /// </summary>
    /// <param name="button"></param>
    private void OnMouseClick(Silk.NET.Input.MouseButton button, int x, int y) => OnMouseClick((MouseButton)button, x, y);
    /// <summary>
    /// Wraps Silk.NET.Input.MouseButton -> RLLib.MouseButton
    /// </summary>
    /// <param name="button"></param>
    private void OnMouseDoubleClick(Silk.NET.Input.MouseButton button, int x, int y) => OnMouseDoubleClick((MouseButton)button, x, y);
    /// <summary>
    /// Wraps Silk.NET.Input.ScrollWheel -> RLLib.ScrollWheel
    /// </summary>
    /// <param name="scrollWheel"></param>
    private void OnMouseScroll(Silk.NET.Input.ScrollWheel scrollWheel) => OnMouseScroll(new ScrollWheel(scrollWheel));
    /// <summary>
    /// Wraps Silk.NET.Input.Key -> RLLib.Key
    /// </summary>
    /// <param name="key"></param>
    private void OnKeyDown(Silk.NET.Input.Key key, int scancode) => OnKeyDown((Key)key, scancode);
    /// <summary>
    /// Wraps Silk.NET.Input.Key -> RLLib.Key
    /// </summary>
    /// <param name="key"></param>
    private void OnKeyUp(Silk.NET.Input.Key key, int scancode) => OnKeyUp((Key)key, scancode);
    #endregion
    #endregion
    ...
    public unsafe GameWindow(int WINDOW_TILE_WIDTH, int WINDOW_TILE_HEIGHT) : base(WINDOW_TILE_WIDTH, WINDOW_TILE_HEIGHT)
    {
        VulkanWindow = new WindowBuilder()
            ...
            .SetWindowState(Silk.NET.Windowing.WindowState.Normal)
    ...
    {
        VkFunc.WaitForFences(VulkanLogicalDevice, new[] { InFlightFences[CurrentFrame].Fence }, true, ulong.MaxValue);
        ...
        {
            VkFunc.WaitForFences(VulkanLogicalDevice, new[] { ImagesInFlight[imageIndex].Fence }, true, ulong.MaxValue);
        }
        ...
        VkFunc.ResetFences(VulkanLogicalDevice, InFlightFences[CurrentFrame]);

        if (VkFunc.QueueSubmit(VulkanGraphicsQueue, InFlightFences[CurrentFrame], submitInfo) != Result.Success)
        {
    ...
    protected void LoadTexture(string textureFilename, int textureTileWidth, int textureTileHeight)
    {
        ...
        VkFunc.DeviceWaitIdle(VulkanLogicalDevice);
    ...
    {
        //VkFunc.ResetDescriptorPool(VulkanLogicalDevice, VulkanDescriptorPool, 0);
        ...
        VkFunc.UpdateDescriptorSets(VulkanLogicalDevice, descriptorWrites, null);
    }
    #endregion
